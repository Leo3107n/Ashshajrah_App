...
export default function AdminMore() {
  ...
  const load = useCallback(async () => {
    setRefreshing(true);
    try {
      const [leads, forms, scholarships, audits] = await Promise.all([
      ...
      setCounts({
        ...
        audits: audits?.summary?.recent || audits?.items?.length || 0,
      });
    } catch {
      // Individual destination screens retain their own retry states.
    } finally {
      setRefreshing(false);
    }
  }, []);
  ...
  return (
    <Screen contentContainerStyle={styles.content} refreshControl={<RefreshControl colors={[colors.gold]} onRefresh={load} refreshing={refreshing} tintColor={colors.gold} />}>
