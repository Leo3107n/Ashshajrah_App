...
export default function TeacherNotifications() {
  ...
  return <Screen contentContainerStyle={styles.content} refreshControl={<RefreshControl colors={[colors.gold]} onRefresh={() => load({ refresh: true })} refreshing={refreshing} tintColor={colors.gold}/>}>
    <View style={styles.heading}><View style={styles.headingCopy}><AppText variant="display">Updates</AppText><AppText style={styles.subtitle}>Your teaching alerts and current school announcements.</AppText></View>{tab === "notifications" && summary.unread ? <Pressable disabled={saving} onPress={confirmMarkAll} style={styles.markAll}><Ionicons color={colors.primary} name="checkmark-done-outline" size={18}/></Pressable> : null}</View>
    ...
    <View style={styles.list}>{tab === "notifications"
      ? items.length ? items.map((item) => <NotificationCard item={item} key={item.id} onPress={() => markRead(item)}/>) : <Empty icon="notifications-off-outline" title="No notifications" text="New teaching updates will appear here."/>
