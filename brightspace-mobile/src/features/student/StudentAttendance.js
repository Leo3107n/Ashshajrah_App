...
export default function StudentAttendance() {
  ...
  const load = useCallback(async ({ refresh = false } = {}) => {
    ...
    setError("");
    try {
      const response = await api.student.attendance();
      setData({ summary: response?.summary || {}, items: response?.items || [] });
    } catch (nextError) {
  ...
  return (
    ...
    >
      ...
      <SurfaceCard style={styles.overview}>
        ...
        <View style={styles.overviewCopy}>
          <AppText style={styles.overviewTitle}>
            {percentage >= 80 ? "Strong consistency" : percentage >= 60 ? "Keep building momentum" : "Attendance needs attention"}
          </AppText>
      ...
      <PillButton
        ...
        onPress={() => router.push("/(app)/student/progress")}
